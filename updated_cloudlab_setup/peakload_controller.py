#!/usr/bin/env python3
"""
peakload_controller.py
───────────────────────
Polls the node-metrics-collector daemonset for per-node and per-pod CPU,
and migrates the highest-load pod off the most overloaded node using peak
load as both the hotspot-detection and victim-selection metric.

Migration mechanics (create -> wait -> drain -> delete) and kubectl/
scraping helpers live in migration_common.py, shared with
avgload_controller.py and tardis_controller.py — edit that file if you
need to change how migrations are actually performed; edit this file only
for peak-load-specific policy (hotspot threshold, victim selection).
